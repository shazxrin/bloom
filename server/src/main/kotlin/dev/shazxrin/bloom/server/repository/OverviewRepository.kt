...
import dev.shazxrin.bloom.server.model.overview.CategoryTotalDuration
import dev.shazxrin.bloom.server.model.overview.DateTotalDuration
import dev.shazxrin.bloom.server.model.task.Task
...
interface OverviewRepository : CrudRepository<Task, String> {
    @Query("SELECT new dev.shazxrin.bloom.server.model.overview.CategoryTotalDuration(c, sum(t.duration)) " +
            "FROM Task AS t " +
    ...
    @Query(
        "SELECT new dev.shazxrin.bloom.server.model.overview.DateTotalDuration(EXTRACT(DAY FROM t.startTime), EXTRACT(MONTH FROM t.startTime), EXTRACT(YEAR FROM t.startTime), sum(t.duration)) " +
            "FROM Task AS t " +
            "WHERE t.startTime BETWEEN :from AND :to " +
